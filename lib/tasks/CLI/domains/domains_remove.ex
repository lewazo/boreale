defmodule Mix.Tasks.CLI.DomainsRemove do
  def run(args) do
    ...
    case args do
      %{"--help" => _} -> Mix.Tasks.CLI.Utils.print_help_for("domains remove")
      x when x == %{} -> remove_domain()
      _ -> IO.puts "boreale cli: domains remove command does not take any arguments"
           IO.puts "See 'boreale cli domains remove --help'"
    end
  end

  defp remove_domain do
    domains = Mix.Tasks.CLI.Domains.run()

    with {:domains_not_empty} <- is_domains_empty?(domains),
         {:ok, domain_name} <- get_domain(domains)
    do
      {:ok, table} =
        File.cwd!
